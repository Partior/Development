...
for itr=1:100
    m_opt_m(itr)=fsolve(@(v) dg2(v,hdom(itr),W0(19)),500,...
        optimoptions('fsolve','Display','off'))/a(hdom(itr));
    m_opt_e(itr)=fsolve(@(v) dg2(v,hdom(itr),W0(0)),400,...
        optimoptions('fsolve','Display','off'))/a(hdom(itr));
end
...
% Stall Speed
CLmax=1.6;
hinter_m=fsolve(@(h) sP(h,sqrt(W0(19)./(1/2*p(h)*S*CLmax)),1,19),40e3,optimoptions('fsolve','Display','off'));
hdom3_m=[hdom(hdom<hinter_m),hinter_m];
v_stall_m=sqrt(W0(19)./(1/2*p(hdom3_m)*S*CLmax));

hinter_e=fsolve(@(h) sP(h,sqrt(W0(0)./(1/2*p(h)*S*CLmax)),1,0),40e3,optimoptions('fsolve','Display','off'));
hdom3_e=[hdom(hdom<hinter_e),hinter_e];
v_stall_e=sqrt(W0(0)./(1/2*p(hdom3_e)*S*CLmax));
% Service Ceiling
V_mp=@(h) (K*W0(19)^2./(3/4*Cd0*p(h).^2*S^2)).^(1/4);                            
h_sc_m=fsolve(@(h) sP(h,V_mp(h),1,19)-100/60,40e3,optimoptions('fsolve','Display','off')); 
h_sc_e=fsolve(@(h) sP(h,V_mp(h),1,0)-100/60,40e3,optimoptions('fsolve','Display','off')); 
