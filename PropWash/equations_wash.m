...
L=@(aoa,h,v,on) (...
    ...
    1/2*p(h)*v2(v,T(v,h,Pa/n),h)^2*Cla((aoa+incd)*ang(v,h))*((2*Rmax*on)*chrd))+... prop wash lift
...
Dw=@(aoa,h,v,on) (...
    ...
    1/2*p(h)*v2(v,T(v,h,Pa/n),h)^2*Cda((aoa+incd)*ang(v,h))*((2*Rmax*on)*chrd)); % prop wash drag
% Total Drag
