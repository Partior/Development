...
ang=@(v,h) v/v2(v,T(v,h,Pa/n),h)*0.75;

% ANGLE OF INCIDENCE
incd=6.7;     % incedence angle of wing strucutre
incd=0;
...
L=@(aoa,h,v,on) (...
    ...
    1/2*p(h)*v2(v,T(v,h,Pa/n),h)^2*Cla(aoa*ang(v,h)+incd)*((2*Rmax*on)*chrd))+... prop wash lift
...
Dw=@(aoa,h,v,on) (...
    ...
    1/2*p(h)*v2(v,T(v,h,Pa/n),h)^2*Cda(aoa*ang(v,h)+incd)*((2*Rmax*on)*chrd)); % prop wash drag
% Total Drag
