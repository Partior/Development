...
Lw=@(aoa,h,v,on) ...
    ...
    1/2*p(h)*v2(v,CT(v,h,Pa,on),h,1).^2*Cla(incd)*(2*s_a(1))+... prop wash lift, cruise props
...
Df=@(aoa,h,v) 1/2*p(h)*v.^2*(Cda(aoa+Cl0)+(K*Cla(aoa+Cl0).^2))*S*1.5; % 80*5 is length * diamter for approx surface area
Dw=@(aoa,h,v,on) ...
    1/2*p(h)*v.^2*Cda(aoa+incd)*...
    (S-2*((on-2)/2>=[0,1,2,3])*s_a)+...free stream wing drag
    1/2*p(h)*v2(v,CT(v,h,Pa,on),h,1).^2*Cda(incd)*(2*s_a(1))+... % prop wash drag, cruise props
